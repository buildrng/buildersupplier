			...
			{({ open }) => (
				<div class="relative ms-2 lg:ms-0 z-10 min-w-[180px]">
					...
					>
						<Listbox.Options
							static
							class="absolute w-full py-1 mt-1 overflow-auto bg-white rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none text-sm"
						>
								...
								>
									{({ selected, active }) => (
										<>
											...
											{selected ? (
												<span
													class={`${active ? "text-amber-600" : ""}
                                check-icon absolute inset-y-0 start-0 flex items-center ps-3`}
												>
													<HiCheck class="w-5 h-5" aria-hidden="true" />
												</span>
											) : null}
										</>
									)}
