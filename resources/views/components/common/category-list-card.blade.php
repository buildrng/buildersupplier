...
const CategoryListCard: React.FC<Props> = ({ category }) => {
	...
	return (
		<Link href={`${ROUTES.CATEGORY}/${category.slug}`}>
			<a class="flex justify-between items-center bg-gray-200 rounded-md px-5 2xl:px-3.5 py-3 xl:py-3.5 2xl:py-2.5 3xl:py-3.5 transition hover:bg-gray-100">
				<div class="flex items-center">
					<div class="inline-flex flex-shrink-0 2xl:w-12 2xl:h-12 3xl:w-auto 3xl:h-auto">
						<Image
							...
							width={60}
							height={60}
							class="bg-gray-300 object-cover rounded-full"
						/>
					</div>
					<h3 class="text-sm md:text-base 2xl:text-sm 3xl:text-base text-heading capitalize ps-2.5 md:ps-4 2xl:ps-3 3xl:ps-4">
						{name}
					</h3>
				</div>
				<div class="flex items-center">
