...
const Instagram: React.FC<Props> = ({ class = "" }) => {
	...
	return (
		...
		>
			{instagramFeed?.map((item) => (
				<a
					class="group flex justify-center text-center relative"
					href={item.slug}
				...
				>
					...
					/>
					<div class="absolute top left bg-black w-full h-full opacity-0 transition-opacity duration-300 group-hover:opacity-50" />
					<div class="absolute top left h-full w-full flex items-center justify-center">
						<FaInstagram class="text-white text-base sm:text-xl md:text-3xl lg:text-5xl xl:text-6xl transform opacity-0 scale-400 transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:scale-100" />
					</div>
				</a>
			))}
		</div>
