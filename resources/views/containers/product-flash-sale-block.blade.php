...
const renderer = ({ days, hours, minutes, seconds, completed }: any) => {
	if (completed) {
		...
		// Render a countdown
		return (
			<div class="flex items-center space-s-1.5 md:space-s-2.5">
				<div class="text-heading text-10px md:text-xs text-center uppercase">
					<span class="bg-heading rounded-md text-white text-xs md:text-sm w-8 h-8 md:w-10 md:h-10 flex items-center justify-center mb-1">
						{days}
					</span>
					days
				</div>
...
const ProductsFlashSaleBlock: React.FC<ProductsProps> = ({
	...
	return (
		<div
			class={`${class} border border-gray-300 rounded-md pt-5 md:pt-6 lg:pt-7 pb-5 lg:pb-7 px-4 md:px-5 lg:px-7`}
		>
			<div class="flex justify-between items-center flex-wrap mb-5 md:mb-6">
				<SectionHeader sectionHeading={sectionHeading} class="mb-0" />
