...
const HeroWithCategoryFlash: React.FC<Props> = ({
	...
	return (
		<div
			class={`grid grid-cols-1 lg:grid-cols-7 2xl:grid-cols-9 gap-5 xl:gap-7 lg:gap-y-14 ${class}`}
		>
			<CategoryListCardSection />

			<div class="col-span-full lg:col-span-5 xl:col-span-5 row-span-full lg:row-auto grid grid-cols-2 gap-2 md:gap-3.5 lg:gap-5 xl:gap-7">
	...
	return (
		<>
			{error ? (
				...
				<div class="col-span-full lg:col-span-2 grid grid-cols-1 gap-3 justify-between">
					{isLoading
						? Array.from({ length: 7 }).map((_, idx) => (
								<CategoryListCardLoader
									key={idx}
		...
		<>
			{width < 1441 ? (
				...
				<SellWithProgress
					...
					imgHeight={425}
					error={error?.message}
					class="col-span-full 2xl:col-span-2 2xl:row-auto xl:hidden 2xl:flex"
				/>
			)}
		</>
	);
