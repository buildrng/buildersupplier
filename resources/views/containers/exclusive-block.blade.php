...
const ExclusiveBlock: React.FC<Props> = ({
	...
	return (
		<div class={`rounded-md overflow-hidden lg:block ${class}`}>
			<div class="flex justify-between">
				{data.exclusiveData.slice(0, 2).map((item: any) => (
					<div
						class={`group w-2/4 flex justify-between items-end relative transition duration-200 ease-in ${
							item.id === 2 ? "flex-row-reverse" : ""
						} ${item.backgroundColor}`}
						key={`exclusive--key${item.id}`}
					>
						<div class="exclusiveImage relative z-10 flex transform transition duration-200 ease-in group-hover:scale-105">
							<Image
								src={item.image}
								alt={item.buttonText}
								width={600}
						...
						{data.exclusiveName && (
							<div
								class={`z-0 absolute top-10 xl:top-12 2xl:top-16 3xl:top-24 uppercase text-black opacity-10 text-xl xl:text-2xl 3xl:text-3xl tracking-widest leading-5 ${
						...
						{data.year && (
							<div
								class={`exclusiveYear absolute top-16 xl:top-20 2xl:top-24 3xl:top-32 start-0 z-10 text-black font-bold leading-none tracking-widest ${
