...
Route::prefix('/v1')->group(function () {
    ...
    // Get All categories
    Route::get('category/getHome/{sector?}', [CategoryController::class, 'getHome']);
    
    // Get Sub-categories
    Route::get('category/getSub', [CategoryController::class, 'getSub']);
    ...
    // Get All Active Stores by city
    Route::post('stores/getStores', [StoresController::class, 'getStoresByCity']);

    // Get All Banners by Store
    Route::post('stores/getBanners', [StoresController::class, 'getBanners']);

    // Get All Offer Products by Store
    Route::post('stores/getOfferProducts', [StoresController::class, 'getOfferProducts']);

    // Get All Top Picked Products by Store
    Route::post('stores/getTopPickedProducts', [StoresController::class, 'getTopPickedProducts']);
    ...
    Route::post('home/getProductsWithSlugs', [ProductsController::class, 'getProductsWithSlugs']);
    Route::post('home/getProducts/{city_slug?}/{categories_slug?}/{sub_categories_slug?}', function ($city_slug = null, $categories_slug = null, $sub_categories_slug = null  ) {
        if ($city_slug) {
        ...
        if ($sub_categories_slug) {
            
        }
    });
    Route::get('home/getProductsWithSlug/{city_id}/{slug?}', function ($city_id, $slug) {
        if ($slug) {
            // $today = Carbon::now();
            // $city = \App\Models\Cities::where('slug', $city_slug)->get();
            ...
            $sub_category = SubCategory::where('slug', $slug)->get();
            if ($sub_category) {
                $products = \App\Models\Products::where('status' , 1)
                ->where('sub_cate_id' , $sub_category->id)
                ->Where('cid', $city_id)
                ->orderBy('rating', 'desc')
                ->get();
    ...
    Route::post('products/getInOffers', [ProductsController::class, 'getInOffers']);
    ...
    Route::group(['namespace' => 'Auth'], function () {
        ...
        Route::post('auth/create_account', [RegisterController::class, 'registerNewCustomer']);
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        // ['middleware' => ['admin_auth', 'jwt.auth']],
    ...
    Route::middleware('auth:sanctum')->group(function () {
        // 'middleware' => ['store_auth', 'jwt.auth']],
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        Route::post('stores/getStoreInfoFromDriver', [StoresController::class, 'getByUID']);
